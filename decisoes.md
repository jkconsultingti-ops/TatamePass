...
## Formulário de perfil: nudge só no convite, não gate bloqueante (revisado)

Antes, o formulário configurado pelo admin (`perfil_campos`) só existia como tela de edição em `/aluno/perfil`, com salvar por campo — a academia configurava campos obrigatórios e nunca recebia resposta, porque nada obrigava o aluno a visitar aquela tela. Primeira tentativa: `PerfilCompletoRoute`, uma rota-gate que envolvia todas as rotas de aluno e redirecionava pra `/aluno/completar-perfil` sempre que `usePerfilCompleto()` (`src/lib/formularios.ts`) desse incompleto — "inclusive em acessos futuros, não só logo após o convite", por pedido explícito do usuário na hora. Na prática isso vazou: qualquer aluno já cadastrado antes de existirem campos obrigatórios (ex: os padrão de CPF/telefone/data de nascimento que `create_academia()` cria desde a 0010) ficava travado nessa tela toda vez que abria o link do app, não só quem tinha acabado de entrar por convite. Revertido — `PerfilCompletoRoute` foi removida do `router.tsx`/`ProtectedRoute.tsx`; `/aluno/completar-perfil` continua existindo e é usada uma vez, logo após `ConviteEntrar.tsx` criar o profile (ver fluxo de convite abaixo), mas não há mais gate persistente prendendo quem já é membro em visitas normais ao app.
...
## Personalização de marca por academia: cor + logo dentro do app, não ícone de instalação

O usuário queria que cada academia pudesse trocar a cor e a "foto" do app pros próprios alunos/professores, inclusive no momento de instalar como PWA. Avaliado e decidido com ele: o ícone que aparece ao "adicionar à tela inicial" vem do `manifest.webmanifest`, que é um arquivo estático servido igual pra qualquer visitante do domínio — o navegador lê isso antes de saber de qual academia é o usuário, então não dá pra ter um ícone de instalação diferente por academia sem cada uma ter subdomínio próprio (infra bem maior: DNS, domínio por cliente). Optamos por só personalizar dentro do app: `academias.cor_marca` (hex) e `academias.logo_url`, carregados pelo `AuthProvider` assim que o perfil resolve a academia, com a cor aplicada via `aplicarCorMarca()` (`src/lib/branding.ts`) sobrescrevendo as CSS custom properties `--color-hanko`/`--color-hanko-dark` em `:root` — como os utilitários Tailwind v4 (`bg-hanko`, `text-hanko` etc) leem a variável em tempo de execução, isso re-tinge o app inteiro sem rebuild. O logo substitui o `Stamp` no cabeçalho (`AppShell.tsx`) e o nome da academia substitui "TatamePass" ali, pra parecer "o app da academia" pra quem já é membro. Editável pelo admin em `/admin/marca`. Ícone de instalação do PWA continua genérico do TatamePass.

## Cor sugerida a partir da logo, e tema claro como preset em vez de gerado livremente

Depois da personalização de cor+logo, o usuário pediu pra também trocar fundo/texto, mas sugeriu algo melhor: extrair a cor automaticamente da logo em vez de escolher tudo na mão. Avaliado com ele: extrair só a cor de destaque da logo é seguro (é só matemática de pixel), mas gerar fundo/texto/borda automaticamente a partir da logo é arriscado — o app tem ~10 variáveis de cor que dependem umas das outras pra manter contraste legível, e um algoritmo ingênuo podia gerar texto escuro em fundo escuro. Decisão: `extrairCorDominante()` (`src/lib/branding.ts`) lê os pixels da logo num canvas, ignora quase-branco/preto/cinza (fundo ou traço, não "a cor"), e sugere a cor mais vibrante/frequente pro campo de cor — o admin ainda pode ajustar. Pro fundo/texto, um tema "claro" fixo e já testado (`:root[data-theme='claro']` em `index.css`, aplicado por `aplicarTema()`) em vez de gerar livre — troca `--color-ink`/`--color-ink-soft`/`--color-chalk`/`--color-rope`/`--color-rope-dim`. Achado no meio do caminho: `--color-ink` também era usado como "tinta escura literal" num widget sempre-claro (calendário do professor, `ProfessorAgenda.tsx`) — precisou virar `--color-ink-fixed` (token que não troca de tema) nesses 6 lugares pra não ficar texto escuro sumindo no próprio fundo claro do widget quando o tema geral vira claro.
